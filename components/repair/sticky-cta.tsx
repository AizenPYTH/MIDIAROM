...
export function StickyCta({ label, priceCents, provisional = false, href, cta }: { label: string; priceCents: number; provisional?: boolean; href: string; cta: string }) {
  return (
    <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-bg p-3 lg:hidden">
      <div className="mx-auto flex max-w-[1280px] items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="font-mono text-[14px] font-semibold text-ink">{formatRepairPrice(priceCents, provisional)}</p>
        </div>
