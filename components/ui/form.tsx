 ...
 */
const inputBase =
  "block w-full rounded-[2px] border border-border-strong bg-field px-4 py-3 text-[16px] text-ink placeholder:text-ink-muted focus:border-accent focus:outline-none disabled:bg-surface-muted disabled:text-ink-muted aria-[invalid=true]:border-danger sm:text-[15px]";
...
export function Checkbox({ className, ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input type="checkbox" className={cn("h-[18px] w-[18px] shrink-0 appearance-none rounded-[2px] border border-border-strong bg-field checked:border-sale checked:bg-sale sm:h-4 sm:w-4", className)} {...props} />;
}
...
export function FormError({ message }: { message?: string | null }) {
  ...
  return (
    <div role="alert" className="rounded-[2px] border border-danger bg-danger-soft px-4 py-3 text-sm text-danger">
      {message}
    </div>
  );
}
...
export function FormSuccess({ message }: { message?: string | null }) {
  if (!message) return null;
  return (
    <div role="status" className="rounded-[2px] border border-success bg-success-soft px-4 py-3 text-sm text-success">
      {message}
    </div>
  );
}
