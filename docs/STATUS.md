...
| Fiches de jeux IGDB (enrichissement des produits « Jeu ») | IMPLEMENTED | Authentification Twitch OAuth côté serveur, cache `igdb_games` (fiche normalisée, 30 j), correspondance notée par plateforme / EAN / édition / année avec validation humaine réversible, API interne `/api/games`, source d'accueil `getHomepageGames()`. **La section « Derniers jeux » de l'accueil attend ses composants** — voir `docs/IGDB.md` |
