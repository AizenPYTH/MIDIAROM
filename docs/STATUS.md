...
| Fiches consoles (`/consoles`, `/consoles/[modèle]`) | REMOVED | Hors cahier des charges : celui-ci ne demande que des « pages SEO par console et panne », déjà servies par `/reparation/[modèle]` et `/reparation/[modèle]/[panne]`. Le choix de la console se fait dans le parcours de devis |
| Catalogues externes (IGDB, HobbyLink Japan / Apify) | REMOVED | Toutes les dépendances de catalogue tiers ont été retirées : plus d'enrichissement de fiches de jeux, plus d'import de figurines, plus de synchronisation automatique. Jeux, consoles et figurines sont saisis à la main depuis `/admin/annonces/nouvelle`, avec leurs photos. Les migrations correspondantes restent appliquées, leurs colonnes simplement inutilisées |
| Boutique (`/boutique`, fiche produit, panier, `/commande-boutique`) | REMOVED | Le client ne vend rien en ligne : pages publiques, panier et tunnel de commande supprimés. Le back-office garde Stock et Commandes pour l'historique, et `lib/shop/*` reste branché sur le retour de paiement |
...
| Espace client : reprises | IMPLEMENTED | `/compte/reprises`. `/compte/commandes` (commandes boutique) a été supprimé avec la vente |
