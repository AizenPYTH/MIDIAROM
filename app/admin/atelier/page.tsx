...
/**
 * L'atelier — un seul écran, une seule tâche.
 *
 * Cette page était `/admin`, c'est-à-dire la porte d'entrée du back-office.
 * Un réparateur y arrivait bien, mais un gérant qui venait mettre une figurine
 * en vente tombait sur une file de réparations et n'avait, pour en sortir,
 * qu'un menu « Plus ». Elle a donc sa propre adresse, et `/admin` est redevenu
 * un accueil qui pose la question « qu'est-ce que vous voulez faire ? ».
 *
 *
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid items-start gap-[18px] [grid-template-columns:minmax(0,1fr)] xl:[grid-template-columns:minmax(330px,1fr)_minmax(380px,1.1fr)]">
        ...
        <section className={cn("min-w-0 flex-col gap-4", explicitSelection ? "hidden xl:flex" : "flex")}>
          <form method="get" action="/admin/atelier" className="flex flex-col gap-3">
            ...
            <>
              ...
              >
                {draftQuote ? (
                  ...
                  <Link href={`/admin/orders/${selected.id}?tab=quotes`} className="btn-gradient flex min-h-11 flex-[1_1_180px] items-center justify-center rounded-[2px] px-5 py-3 text-center text-[14px] font-semibold">
                    Envoyer le devis
                  </Link>
                )}
                <Link
