...
const SETTINGS_SCHEMAS: Record<string, z.ZodType<Record<string, unknown>>> = {
  brand: z.object({ name: z.string().trim().min(1).max(80), tagline: z.string().trim().max(120), description: z.string().trim().max(500), email: z.string().trim().email().or(z.literal("")), phone: z.string().trim().max(30), address_line1: z.string().trim().max(120), postal_code: z.string().trim().max(10), city: z.string().trim().max(80), country: z.string().trim().max(60), hours: z.string().trim().max(200), siret: z.string().trim().max(20), legal_form: z.string().trim().max(60), logo_path: z.string().trim().max(300).transform((v) => v || null) }),
